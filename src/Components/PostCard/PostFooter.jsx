...
export default function PostFooter({ post }) {
    ...
    const { data: comments, isLoading: CommentsLoading, refetch: getPostComments } = useQuery({
    ...
    const { mutate: updateCommentMutate, isPending: updateCommentPending } = useMutation({
        mutationFn: ({ commentContent, commentId }) => updateCommentApi(commentContent, commentId),
        onSuccess: () => {
            queryClient.invalidateQueries(['getPostComments', post.id])
            setCommentContent('')
    ...
    return (
        <>
            ...
            <div className="addCommentForm relative">
                <div>
                    <Input disabled={addCommentPending || updateCommentPending} className='my-3 text-slate-950 dark:text-white' color='default' variant='faded' placeholder='add comment' value={commentContent} onChange={(e) => setCommentContent(e.target.value)}></Input>
                    {isEditing ? <Button color='warning' variant='faded' isLoading={updateCommentPending} className='absolute right-0 top-1/2 -translate-y-1/2 border-0' onPress={updateComment}>Update <IoMdSend className='text-2xl' /></Button> : <Button isLoading={addCommentPending} className='absolute right-0 top-1/2 -translate-y-1/2 bg-transparent' onPress={addComment}><IoMdSend className='text-2xl cursor-pointer text-[#1D293D]' /></Button>}
                </div>
            </div>
                        ...
                        <div className='bg-slate-300 dark:bg-slate-900 py-1 ps-4 pe-6 rounded-xl'>
                            <div className="flex">
                                ...
                                {userData?._id === post?.user?._id && userData?._id === comments?.[0]?.commentCreator?._id &&
                                    <DropDownCommentComponent comment={comments[0]} handleUpdateComment={handleUpdateComment} getPostCommentsFunction={getPostComments} post={post} />}
                            </div>
